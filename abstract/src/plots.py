...
params = {'backend': 'ps',
          ...
          'figure.figsize': (fig_width_pt * inches_per_pt,
                             fig_width_pt * inches_per_pt * 1.75)}
...
for i, row in enumerate(['Ankle', 'Knee', 'Hip']):
    for j, (col, unit) in enumerate(zip(['Angle', 'Rate'],
        ...
        for side, marker, color in zip(['Right', 'Left'],
            ...
            #axes[i, j].errorbar(percent_of_gait_cycle, gains_per, yerr=sigma,
                                #marker=marker,
                                #ms=2,
                                #color=color,
                                #mfc='black')
            ...
            if i == 2:
                axes[i, j].set_xlabel(r'\% Right Gait Cycle')
                axes[i, j].xaxis.set_major_formatter(_percent_formatter)
                axes[i, j].set_xlim(xlim)

plt.tight_layout()

fig.savefig('../fig/gains.pdf')
